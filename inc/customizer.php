...
function temzap_customize_register( $wp_customize ) {
    //Style Preset
    $wp_customize->add_section(
        'typography',
        array(
            'title' => __( 'Preset Styles', 'temzap' ),
            //'description' => __( 'This is a section for the typography', 'temzap' ),
            'priority' => 20,
        )
    );
    //Theme Option
    ...
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_visibility', array(
        ...
        'label'    => __('Remove Header Banner', 'temzap'),
        'section'    => 'header_image',
        'type'     => 'checkbox',
    ) ) );
   ...
   $wp_customize->add_section(
        ...
        array(
            'title' => __( 'Other Customizations', 'temzap' ),
            //'description' => __( 'This is a section for the header banner Image.', 'temzap' ),
            'priority' => 40,
        )
    );
    $wp_customize->add_section(
        'colors',
        array(
            'title' => __( 'Background Color', 'temzap' ),
            //'description' => __( 'This is a section for the header banner Image.', 'temzap' ),
            'priority' => 50,
    ...
    $wp_customize->add_section(
        'background_image',
        array(
            'title' => __( 'Background Image', 'temzap' ),
            //'description' => __( 'This is a section for the header banner Image.', 'temzap' ),
            'priority' => 60,
            'panel' => 'styling_option_panel',
        )
    ...
    $wp_customize->add_setting( 'temzap_logo', array(
        //'default' => __( '', 'temzap' ),
        'sanitize_callback' => 'esc_url',
    ) );
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'temzap_logo', array(
        'label'    => __( 'Upload Logo (replaces text)', 'temzap' ),
        'section'  => 'title_tagline',
        'settings' => 'temzap_logo',
    ) ) );
}
add_action( 'customize_register', 'temzap_customize_register' );
add_action( 'wp_head', 'temzap_customizer_css');
function temzap_customizer_css()
{
    ?>
    <style type="text/css">
 ...
 */
function temzap_customize_preview_js() {
    wp_enqueue_script( 'temzap_customizer', get_template_directory_uri() . '/inc/assets/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'temzap_customize_preview_js' );
